...
# stan_model = stan_model("ADM1.stan")
# saveRDS(stan_model, "ADM1_compiled.RData")
stan_model = readRDS("ADM1_compiled.RData")
...
for (subjID in subj_list) {
  ...
  data = dataAll %>%
    ...
    mutate(sqrt_time = sqrt(1:n() * (1/30))) %>% 
    ungroup()
  ...
  initf <- function() {list(e_p = runif(2, 0.2, 0.6), 
                            e_r = runif(2, 0.2, 0.6),
  ...
  saveRDS(model_out, paste("ADM1", "fit", sprintf("ADM1_S%02d_fit.RData", subjID),
                           sep = "/"))
}
